  ...
  */
  const prepareDataForBackend = (): FormData => {
    ...
    const screens = uploadedImages.map((image) => ({
      id: image.id,
      image: image.filename, // Filename to match with uploaded files
      isTheStart: image.id === savedFlowMapImages[0]?.id,
    ...
    // Append screens data as JSON string
    data.append('screens', JSON.stringify(screens));

    // Append additional metadata
    data.append('coreId', 'user-core-id'); // Replace with actual user ID
    data.append('testProjectId', 'test-project-id'); // Replace with actual project ID
  ...
  const handleFinishSetUp = async () => {
    ...
    try {
      // Call prepareDataForBackend to get the FormData object
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to submit annotations.');
      }
  ...
  const renderInterfaceItem = (image: UploadedImage, index: number) => {
    ...
    return (
      <div
        key={image.id}
        className={`flex flex-col w-[93px] p-[10px_4px_0px_4px] items-center gap-[1px] rounded-[7px] ${isSelected ? 'border-[1.5px] border-[#625AFA]' : 'border border-[#505050]'
    ...
    <div className="flex flex-col w-full min-h-screen bg-white">
      ...
      <main className="flex flex-1 bg-[#0D1117]">
        ...
        <aside className="w-96 border-l border-[#333] bg-[#1C1D1F] p-3 flex flex-col relative">
          ...
          />
          <button
            className={`mt-auto py-2 px-4 rounded ${hasAnnotations
